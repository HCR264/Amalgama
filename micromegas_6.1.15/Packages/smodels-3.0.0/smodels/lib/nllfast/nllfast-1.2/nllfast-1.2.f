...
c are for LHC production cross sections at 7 TeV collision energy.
c
...
c production processes (or sum of the four):
c
...
c      tot    sum of all subprocesses for squark and gluino production, without stops,
c  - pdf:
...
c... check the masses 
     ...
     &            .AND.((MGL.LT.MGLLO).OR.(MGL.GT.MGLUP))) THEN
        ...
        IF ((PR.EQ.'gg').AND.((MSQ.GT.2.d3).AND.(MGL.GT.1.5d3))) THEN
            write(*,*) 'TOO HIGH SQUARK MASS FOR THIS GLUINO MASS'
            STOP
        ENDIF

        IF ((PR.EQ.'sg').AND.((MSQ.GT.2.d3).AND.(MGL.GT.1.5d3))) THEN
            write(*,*) 'TOO HIGH SQUARK MASS FOR THIS GLUINO MASS'
            STOP
        ENDIF
...
c... stops and squarks in the decoupling limit:
      ...
      IF ((PR.EQ.'st').OR.(PR.EQ.'sdcpl')) THEN  
         write(*,*)
         IF (PDF.EQ.'mstw') THEN
          write(*,*) '# LHC @ 7 TeV, MSTW2008NLO (MSTW2008LO for LO)'
         ELSEIF (PDF.EQ.'cteq') THEN
          write(*,*) '# LHC @ 7 TeV, CTEQ6.6M (CTEQ6L1 for LO)' 
         ENDIF 
         write(*,*) '# process: ', PR
     ...
     &---------------'
         ...
         write(23,*)
         IF (PDF.EQ.'mstw') THEN
          write(23,*) '# LHC @ 7 TeV, MSTW2008NLO (MSTW2008LO for LO)'
         ELSEIF (PDF.EQ.'cteq') THEN
          write(23,*) '# LHC @ 7 TeV, CTEQ6.6M (CTEQ6L1 for LO)' 
         ENDIF
         write(23,*) '# process: ', PR
