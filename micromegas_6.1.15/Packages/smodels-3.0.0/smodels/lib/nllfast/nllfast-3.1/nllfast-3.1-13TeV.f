...
c      nnpdf  NNPDF3.0NLO (NNPDF3.0LO for LO predictions) parton distribution functions,
...
c
c  For the NNPDF3.0 output, the 1-sigma PDF and alpha_s uncertainties (of the NLO+NLL value)
c   are shown in a combined format corresponding to the PDF4LHC recommendations.
c
c Comments:
...
c    up to 4% when gluino mass is bigger than stop_1 mass.
...
c... read in data
     ...
     &                 PDFGRPL(I,J),NLLGR(I,J),NLLGRPL(I,J),
     &                 NLLGRMI(I,J),ASGRPL(I,J)
                  PDFGRPL(I,J) = ASGRPL(I,J)
                  PDFGRMI(I,J) = ASGRPL(I,J)
                  ASGRMI(I,J) = ASGRPL(I,J)
...
c... write out results:
      IF (PDF.eq.'nnpdf') THEN
       IF (PR.EQ.'gdcpl') THEN
            ...
            WRITE(*,15) MGL,DEXP(LO),DEXP(NLO),DEXP(NLL),
     &           DEXP(NLLPL),-DEXP(NLLMI),PDFPL,-PDFMI,
     &           DEXP(NLO)/DEXP(LO),DEXP(NLL)/DEXP(NLO)
            WRITE(23,16) MGL,DEXP(LO),DEXP(NLO),DEXP(NLL),
     &           DEXP(NLLPL),-DEXP(NLLMI),PDFPL,-PDFMI,
     &           DEXP(NLO)/DEXP(LO),DEXP(NLL)/DEXP(NLO)
        ...
        ELSEIF ((PR.EQ.'st').OR.(PR.EQ.'sdcpl')) THEN
            ...
            WRITE(*,15) MSQ,DEXP(LO),DEXP(NLO),DEXP(NLL),
     &           DEXP(NLLPL),-DEXP(NLLMI),PDFPL,-PDFMI,
     &           DEXP(NLO)/DEXP(LO),DEXP(NLL)/DEXP(NLO)
            WRITE(23,16) MSQ,DEXP(LO),DEXP(NLO),DEXP(NLL),
     &           DEXP(NLLPL),-DEXP(NLLMI),PDFPL,-PDFMI,
     &           DEXP(NLO)/DEXP(LO),DEXP(NLL)/DEXP(NLO)

        ELSE

            WRITE(*,17) MSQ,MGL,DEXP(LO),DEXP(NLO),DEXP(NLL),
     &           DEXP(NLLPL),-DEXP(NLLMI),PDFPL,-PDFMI,
     &           DEXP(NLO)/DEXP(LO),DEXP(NLL)/DEXP(NLO)
            WRITE(23,18) MSQ,MGL,DEXP(LO),DEXP(NLO),DEXP(NLL),
     &           DEXP(NLLPL),-DEXP(NLLMI),PDFPL,-PDFMI,
     &           DEXP(NLO)/DEXP(LO),DEXP(NLL)/DEXP(NLO)
     ...
     &           DEXP(NLLPL),-DEXP(NLLMI),PDFPL,-PDFMI,ASPL,-ASMI,
     &           DEXP(NLO)/DEXP(LO),DEXP(NLL)/DEXP(NLO)
            WRITE(23,14) MSQ,MGL,DEXP(LO),DEXP(NLO),DEXP(NLL),
     &           DEXP(NLLPL),-DEXP(NLLMI),PDFPL,-PDFMI,ASPL,-ASMI,
     &           DEXP(NLO)/DEXP(LO),DEXP(NLL)/DEXP(NLO)
...
C for NNPDF
 15   FORMAT (' ',1(F7.0,2x),3(E11.3),'  ',(E11.3),' ',(E11.3),(f8.1),
     &     '     ',(f8.1),'   ',2(f8.2))
 16   FORMAT (' ',1(F7.0,2x),3(E11.3),' ',(E11.3),' ',(E11.3),(f8.1),
     &     '     ',(f8.1),'   ',2(f8.2))
 17   FORMAT (' ',2(F7.0,2x),3(E11.3),'  ',(E11.3),' ',(E11.3),(f8.1),
     &     '     ',(f8.1),'    ',2(f8.2))
 18   FORMAT (' ',2(F7.0,2x),3(E11.3),' ',(E11.3),' ',(E11.3),(f8.1),
     &     '     ',(f8.1),'    ',2(f8.2))
...
c... stops and squarks in the decoupling limit:
      ...
      IF ((PR.EQ.'st').OR.(PR.EQ.'sdcpl')) THEN  
         write(*,*)
         IF (PDF.EQ.'mstw') THEN
          write(*,*) '# LHC @ 13 TeV, MSTW2008NLO (MSTW2008LO for LO)'
         ELSEIF (PDF.EQ.'cteq') THEN
          write(*,*) '# LHC @ 13 TeV, CTEQ6.6M (CTEQ6L1 for LO)'
         ELSEIF (PDF.EQ.'nnpdf') THEN
          write(*,*) '# LHC @ 13 TeV, NNPDF3.0NLO (NNPDF3.0LO for LO)'
         ENDIF 
         write(*,*) '# process: ', PR
         IF (PDF.EQ.'nnpdf') THEN
          write(*,*) '# ms[GeV]  LO[pb]     NLO[pb]    NLL+NLO[pb]  d_mu
     &+[pb]  d_mu-[pb]   d_pdfas+[%]  d_pdfas-[%]  K_NLO   K_NLL'
         ELSE
          write(*,*) '# ms[GeV]  LO[pb]     NLO[pb]    NLL+NLO[pb]  d_mu
     ...
     &---------------'
         ...
         write(23,*)
         IF (PDF.EQ.'mstw') THEN
          write(23,*) '# LHC @ 13 TeV, MSTW2008NLO (MSTW2008LO for LO)'
         ELSEIF (PDF.EQ.'cteq') THEN
          write(23,*) '# LHC @ 13 TeV, CTEQ6.6M (CTEQ6L1 for LO)'
         ELSEIF (PDF.EQ.'nnpdf') THEN
          write(23,*) '# LHC @ 13 TeV, NNPDF3.0NLO (NNPDF3.0LO for LO)'
         ENDIF
         write(23,*) '# process: ', PR
         IF (PDF.EQ.'nnpdf') THEN
          write(23,*) '# ms[GeV]  LO[pb]     NLO[pb]    NLL+NLO[pb]  d_m
     &u+[pb]  d_mu-[pb]   d_pdfas+[%]  d_pdfas-[%]  K_NLO   K_NLL'
         ELSE
          write(23,*) '# ms[GeV]  LO[pb]     NLO[pb]    NLL+NLO[pb]  d_m
     &u+[pb]  d_mu-[pb]    d_pdf+[%]  d_pdf-[%]  d_as+[%] d_as-[%]  K_NL
     &O   K_NLL'
         ENDIF
         write(23,*) '------------------------------------------------------
     &------------------------------------------------------------------
...
c... gluinos in the decoupling limit:         
       ...
       ELSEIF  (PR.EQ.'gdcpl') THEN  
         ...
         write(*,*) '# process: ', PR
         IF (PDF.EQ.'nnpdf') THEN
          write(*,*) '# mg[GeV]  LO[pb]     NLO[pb]    NLL+NLO[pb]  d_mu
     &+[pb]  d_mu-[pb]   d_pdfas+[%]  d_pdfas-[%]  K_NLO   K_NLL'
         ELSE
          write(*,*) '# mg[GeV]  LO[pb]     NLO[pb]    NLL+NLO[pb]  d_mu
     &+[pb]  d_mu-[pb]    d_pdf+[%]  d_pdf-[%]  d_as+[%] d_as-[%]  K_NLO
     ...
     &---------------'
         ...
         write(23,*) '# process: ', PR
         IF (PDF.EQ.'nnpdf') THEN
          write(23,*) '# mg[GeV]  LO[pb]     NLO[pb]    NLL+NLO[pb]  d_m
     &u+[pb]  d_mu-[pb]   d_pdfas+[%]  d_pdfas-[%]  K_NLO   K_NLL'
         ELSE
          write(23,*) '# mg[GeV]  LO[pb]     NLO[pb]    NLL+NLO[pb]  d_m
     &u+[pb]  d_mu-[pb]    d_pdf+[%]  d_pdf-[%]  d_as+[%] d_as-[%]  K_NL
...
c... other processes:
       ELSE
         ...
         write(*,*) '# process: ', PR
         IF (PDF.EQ.'nnpdf') THEN
          write(*,*) '# ms[GeV]  mg[GeV]  LO[pb]     NLO[pb]    NLL+NLO[
     &pb]  d_mu+[pb]  d_mu-[pb]   d_pdfas+[%]  d_pdfas-[%]   K_NLO   K_N
     &LL'
         ELSE
          write(*,*) '# ms[GeV]  mg[GeV]  LO[pb]     NLO[pb]    NLL+NLO[
     &pb]  d_mu+[pb]  d_mu-[pb]   d_pdf+[%]  d_pdf-[%]  d_as+[%] d_as-[%
     &]  K_NLO   K_NLL'
         ENDIF
         write(*,*) '------------------------------------------------------
     &------------------------------------------------------------------
     &---------------'
         ...
         write(23,*) '# process: ', PR
         IF (PDF.EQ.'nnpdf') THEN
          write(23,*) '# ms[GeV]  mg[GeV]  LO[pb]     NLO[pb]    NLL+NLO
     &[pb]  d_mu+[pb]  d_mu-[pb]   d_pdfas+[%]  d_pdfas-[%]   K_NLO   K_
     &NLL'
         ELSE
          write(23,*) '# ms[GeV]  mg[GeV]  LO[pb]     NLO[pb]    NLL+NLO
     &[pb]  d_mu+[pb]  d_mu-[pb]   d_pdf+[%]  d_pdf-[%]  d_as+[%] d_as-[
...
c... open the grid file
       IF (PDF.EQ.'mstw') THEN  
         IF (PR.EQ.'sb') THEN
            open(21,file="sb_nllnlo_mstw2008_13TeV.grid",
     &           status='unknown')
         ELSEIF (PR.EQ.'ss') THEN
            open(21,file="ss_nllnlo_mstw2008_13TeV.grid",
     &           status='unknown')
         ELSEIF (PR.EQ.'gg') THEN
            open(21,file="gg_nllnlo_mstw2008_13TeV.grid",
     &           status='unknown')
            open(31,file="gg_nllnlo_hm_mstw2008_13TeV.grid",
     &           status='unknown')
         ELSEIF (PR.EQ.'sg') THEN
            open(21,file="sg_nllnlo_mstw2008_13TeV.grid",
     &           status='unknown')
            open(31,file="sg_nllnlo_hm_mstw2008_13TeV.grid",
     &           status='unknown')
        ELSEIF (PR.EQ.'st') THEN
            open(21,file="st_nllnlo_mstw2008_13TeV.grid",
     &           status='unknown')
         ELSEIF (PR.EQ.'tot') THEN
            open(21,file="tot_nllnlo_mstw2008_13TeV.grid",
     &           status='unknown')
         ELSEIF (PR.EQ.'sdcpl') THEN
            open(21,file="sdcpl_nllnlo_mstw2008_13TeV.grid",
     &           status='unknown')
         ELSEIF (PR.EQ.'gdcpl') THEN
            open(21,file="gdcpl_nllnlo_mstw2008_13TeV.grid",
     &           status='unknown')
         ELSE   
            write(*,*) 'WRONG INPUT PROCESS'
            STOP
        ...
        ELSEIF (PDF.EQ.'cteq') THEN
         IF (PR.EQ.'sb') THEN
            open(21,file="sb_nllnlo_cteq66_13TeV.grid",
     &           status='unknown')
         ELSEIF (PR.EQ.'ss') THEN
            open(21,file="ss_nllnlo_cteq66_13TeV.grid",
     &           status='unknown')
         ELSEIF (PR.EQ.'gg') THEN
            open(21,file="gg_nllnlo_cteq66_13TeV.grid",
     &           status='unknown')
            open(31,file="gg_nllnlo_hm_cteq66_13TeV.grid",
     &           status='unknown')
         ELSEIF (PR.EQ.'sg') THEN
            open(21,file="sg_nllnlo_cteq66_13TeV.grid",
     &           status='unknown')
            open(31,file="sg_nllnlo_hm_cteq66_13TeV.grid",
     &           status='unknown')
         ELSEIF (PR.EQ.'st') THEN
             open(21,file="st_nllnlo_cteq66_13TeV.grid",
     &            status='unknown')
         ELSEIF (PR.EQ.'tot') THEN
            open(21,file="tot_nllnlo_cteq66_13TeV.grid",
     &           status='unknown')
         ELSEIF (PR.EQ.'sdcpl') THEN
            open(21,file="sdcpl_nllnlo_cteq66_13TeV.grid",
     &           status='unknown')
         ELSEIF (PR.EQ.'gdcpl') THEN
            open(21,file="gdcpl_nllnlo_cteq66_13TeV.grid",
     &           status='unknown')
         ELSE
            write(*,*) 'WRONG INPUT PROCESS'
            STOP
        ...
        ELSEIF (PDF.EQ.'nnpdf') THEN
         IF (PR.EQ.'sb') THEN
            open(21,file="sb_nllnlo_nnpdf30_13TeV.grid",
     &           status='unknown')
         ELSEIF (PR.EQ.'ss') THEN
            open(21,file="ss_nllnlo_nnpdf30_13TeV.grid",
     &           status='unknown')
         ELSEIF (PR.EQ.'gg') THEN
            open(21,file="gg_nllnlo_nnpdf30_13TeV.grid",
     &           status='unknown')
            open(31,file="gg_nllnlo_hm_nnpdf30_13TeV.grid",
     &           status='unknown')
         ELSEIF (PR.EQ.'sg') THEN
            open(21,file="sg_nllnlo_nnpdf30_13TeV.grid",
     &           status='unknown')
            open(31,file="sg_nllnlo_hm_nnpdf30_13TeV.grid",
     &           status='unknown')
         ELSEIF (PR.EQ.'st') THEN
             open(21,file="st_nllnlo_nnpdf30_13TeV.grid",
     &            status='unknown')
         ELSEIF (PR.EQ.'tot') THEN
            open(21,file="tot_nllnlo_nnpdf30_13TeV.grid",
     &           status='unknown')
         ELSEIF (PR.EQ.'sdcpl') THEN
            open(21,file="sdcpl_nllnlo_nnpdf30_13TeV.grid",
     &           status='unknown')
         ELSEIF (PR.EQ.'gdcpl') THEN
            open(21,file="gdcpl_nllnlo_nnpdf30_13TeV.grid",
     &           status='unknown')
         ELSE
            write(*,*) 'WRONG INPUT PROCESS'
            STOP
